...
public static class ContentAccessorExtensions
{
        ...
        IClock clock, string? createdBy, int unitIdsMaxLength = 3900)
    {
        ...
        {
            var paneUnits = accessor.PaneUnits.LocalOrDbWhere(p => p.PaneId == paneId && p.UnitIds.Length < unitIdsMaxLength).ToList();
        ...
        IIdentificationGeneratorFactory idGeneratorFactory, IClock clock, string? createdBy)
    {
        ...
        {
            var tag = accessor.Tags.LocalOrDbFirstOrDefault(p => p.Name == tagName);
            if (tag is null)
            {
                tag = new Tag
                {
